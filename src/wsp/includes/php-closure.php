<?php
/**
 * Description of PHP file wsp\includes\php-closure.php
...
class PhpClosure {
  ...
  function compressionGoogle() {
  	$this->_compression = "GOOGLE_WS";
  	return $this;
  }
  
  function compressionLocal() {
  	$this->_compression = "LOCAL";
  	return $this;
  }
  
  function compressionNone() {
  ...
  function _compile() {
    ...
    if ($this->_compression == "GOOGLE_WS") {
	    ...
	    $result .= "$code \r\n";
    } else if($this->_compression == "LOCAL") {
	    // add by WSP framework to have a local compression
	    $result = JSMinPlus::minify($this->_readSources());
    } else {
    	$result = $this->_readSources();
    }
