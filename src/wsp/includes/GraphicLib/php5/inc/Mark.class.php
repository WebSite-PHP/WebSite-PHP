...
class awMark {
	...
	public function setTypeIndex($index, $type) {
		$this->typeIdx[$index] = (int)$type;
	}
	...
	public function setFillIndex($index, $fill) {
		if($fill instanceof awColor or $fill instanceof awGradient) {
			$this->fillIdx[$index] = $fill;
		}
	}
	 ...
	 */
	public function draw(awDriver $driver, awPoint $point, $first_call = true) {
		...
		if($this->type !== NULL) {
			...
			if (isset($this->typeIdx[$this->typeInd])) {
			   if ($this->typeIdx[$this->typeInd] != "") {
			     $typeSearch = $this->typeIdx[$this->typeInd];
			   }
      }
      if (isset($this->fillIdx[$this->typeInd])) {
			   if ($this->fillIdx[$this->typeInd] != null) {
			     $this->setFill($this->fillIdx[$this->typeInd]);
			   }
      }
      if ($first_call == true) {
			   $this->typeInd++;
			...
			switch($typeSearch) {
				...
				case awMark::STAR :
					...
					$borderVisible = $this->border->visible();
					$this->border->hide();
					$this->draw($driver, $point, false);
