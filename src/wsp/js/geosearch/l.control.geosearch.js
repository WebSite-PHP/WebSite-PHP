...
L.Control.GeoSearch = L.Control.extend({
    ...
    geosearch: function (qry, callback_fct) {
        try {
            var provider = this._config.provider;

            if(typeof provider.GetLocations == 'function') {
                var results = provider.GetLocations(qry, function(results) {
                    this._processResults(results, callback_fct);
                }.bind(this));
            }
            else {
                var url = provider.GetServiceUrl(qry);
                this.sendRequest(provider, url, callback_fct);
            }
        }
        catch (error) {
    ...
    sendRequest: function (provider, url, callback_fct) {
        var that = this;

        window.parseLocation = function (response) {
            var results = provider.ParseJSON(response);
            that._processResults(results, callback_fct);
    ...
    _processResults: function(results, callback_fct) {
        if (results.length > 0) {
            //this._map.fireEvent('geosearch_foundlocations', {Locations: results});
            if (callback_fct == null) {
            	this._showLocation(results[0]);
            } else {
            	callback_fct(results[0]);
            }
        } else {
